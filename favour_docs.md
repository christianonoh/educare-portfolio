...
```css
.advertBanner img {
    position: absolute; /* Takes the image out of the normal layout flow */
    bottom: 0;          /* Positions the image flush with the bottom edge */
    right: 0;           /* Positions the image flush with the right edge */
...
.linksList > :first-child {
    font-weight: bold; /* Makes the first element (the title) bold */
}
